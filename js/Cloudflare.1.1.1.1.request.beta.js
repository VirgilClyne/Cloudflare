...
const $ = new Env("☁ Cloudflare: 1️⃣ 1.1.1.1 v3.0.0(1).request.beta");
const URL = new URLs();
const DataBase = {
	"Panel": {
		...
		default:
			...
			const KIND = RegExp(`/reg/${Settings.Verify.RegistrationId}$`, "i").test($request.url) ? "RegistrationId"
				: /reg/i.test($request.url) ? "Registration"
					: undefined
			*/
			const KIND = `/${PATHs[PATHs.length - 2]}/${PATHs[PATHs.length - 1]}` === `/reg/${Settings.Verify.RegistrationId}$` ? "RegistrationId"
				: `/${PATHs[PATHs.length - 2]}` === `/reg` ? "Registration"
					: undefined;
			$.log(`🚧 ${$.name}`, `KIND: ${KIND}`, "");
